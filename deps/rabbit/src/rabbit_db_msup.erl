...
delete(Group, {SimpleId, _} = Id) ->
    rabbit_db:run(
      #{mnesia => fun() -> delete_in_mnesia(Group, Id) end,
        khepri => fun() -> delete_in_khepri(Group, SimpleId) end
       }).
...
delete_in_khepri(Group, SimpleId) ->
...
find_mirror(Group, {SimpleId, _} = Id) ->
    ...
    rabbit_db:run(
      #{mnesia => fun() -> find_mirror_in_mnesia(Group, Id) end,
        khepri => fun() -> find_mirror_in_khepri(Group, SimpleId) end
       }).
...
find_mirror_in_khepri(Group, SimpleId) ->
