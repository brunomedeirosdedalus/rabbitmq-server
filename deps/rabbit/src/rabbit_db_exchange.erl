...
list_in_khepri() ->
    case rabbit_khepri:match(khepri_exchanges_path() ++
                                 [rabbit_db:if_has_data_wildcard()]) of
        {ok, Map} ->
...
update_in_khepri(#resource{virtual_host = VHost, name = Name} = XName, Fun) ->
...
insert(Xs) ->
    rabbit_db:run(
      #{mnesia => fun() -> insert_in_mnesia(Xs) end,
        khepri => fun() -> insert_in_khepri(Xs) end
       }).
...
insert_in_khepri(Xs) when is_list(Xs) ->
    rabbit_khepri:transaction(
      fun() ->
              [insert_in_khepri_tx(X) || X <- Xs]
      end, rw),
    ok.
...
next_serial(X) ->
    rabbit_db:run(
      #{mnesia => fun() -> next_serial_in_mnesia(X) end,
        khepri => fun() -> next_serial_in_khepri(X) end
       }).
...
next_serial_in_khepri(#exchange{name = #resource{name = Name, virtual_host = VHost} = XName} = X) ->
...
recover_in_khepri(VHost) ->
    ...
    rabbit_khepri:transaction(
      fun() ->
              [_ = insert_in_khepri_tx(X) || X <- Exchanges]
