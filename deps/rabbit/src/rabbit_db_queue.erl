...
get_all_in_khepri(VHostName) ->
    list_with_possible_retry_in_khepri(
      fun() ->
              rabbit_db:list_in_khepri(khepri_queues_path() ++ [VHostName, rabbit_db:if_has_data_wildcard()])
      end).
...
filter_all_durable_in_khepri(FilterFun) ->
    Path = khepri_queues_path() ++ [rabbit_db:if_has_data_wildcard()],
...
-spec update_decorators(QName, {[Decorator], [Decorator]}) -> ok when
      QName :: rabbit_amqqueue:name(),
      Decorator :: atom().
...
get_all_by_type_and_node_in_khepri(VHostName, Type, Node) ->
    Pattern = amqqueue:pattern_match_on_type(Type),
    Qs = rabbit_db:list_in_khepri(khepri_queues_path() ++ [VHostName, rabbit_db:if_has_data([?KHEPRI_WILDCARD_STAR_STAR, #if_data_matches{pattern = Pattern}])]),
